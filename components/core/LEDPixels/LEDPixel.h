...
public:
    ...
    // Set from RGB
    inline void fromRGB(uint8_t r_in, uint8_t g_in, uint8_t b_in, ColourOrder order) ALWAYS_INLINE
    {
        switch(order)
        {
            case RGB:
    ...
    inline void fromRGB(uint32_t rgb24bit, ColourOrder order) ALWAYS_INLINE
    {

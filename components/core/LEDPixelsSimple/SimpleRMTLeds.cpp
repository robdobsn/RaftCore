...
// Create LED strip encoder with configured timing
esp_err_t SimpleRMTLeds::createEncoder()
{
    ...
    // Use configured timing values (already in RMT ticks)
    rmt_bytes_encoder_config_t bytes_encoder_config = {};
    bytes_encoder_config.bit0.level0 = 1;
    bytes_encoder_config.bit0.duration0 = _T0H_ticks;
    bytes_encoder_config.bit0.level1 = 0;
    bytes_encoder_config.bit0.duration1 = _T0L_ticks;
    bytes_encoder_config.bit1.level0 = 1;
    bytes_encoder_config.bit1.duration0 = _T1H_ticks;
    bytes_encoder_config.bit1.level1 = 0;
    bytes_encoder_config.bit1.duration1 = _T1L_ticks;
    bytes_encoder_config.flags.msb_first = _msbFirst ? 1 : 0;
    ...
    led_encoder->reset_code.duration0 = _reset_ticks;
    led_encoder->reset_code.level1 = 0;
    led_encoder->reset_code.duration1 = 0;
    
    _encoder = &led_encoder->base;
    return ESP_OK;
}
                          ...
                          bool msbFirst)
{
    ...
    tx_chan_config.trans_queue_depth = 1;                   // Only one transaction at a time (queueing without data buffering causes corruption)
    ...
    _initialized = true;
    LOG_I(MODULE_PREFIX, "Initialized pin=%d pixels=%d res=%dHz T0H=%d T0L=%d T1H=%d T1L=%d rst=%d msb=%d", 
          pin, numPixels, _rmtResolutionHz, _T0H_ticks, _T0L_ticks, _T1H_ticks, _T1L_ticks, _reset_ticks, _msbFirst);
    return true;
}

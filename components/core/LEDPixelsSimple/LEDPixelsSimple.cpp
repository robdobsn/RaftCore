// LEDPixelsSimple.cpp
// Custom LEDPixelsSimple implementation that uses SimpleRMTLeds driver instead of ESP32RMTLedStrip
// Rob Dobson 2024

#include "LEDPixelsSimple.h"
#include "RaftUtils.h"
...
LEDPixelsSimple::LEDPixelsSimple()
{
}
...
bool LEDPixelsSimple::setup(const RaftJsonIF& config)
{
    // LED pixels config
...
bool LEDPixelsSimple::setup(LEDPixelConfig& config)
{
    // Setup pixels
...
void LEDPixelsSimple::addPattern(const String& patternName, LEDPatternCreateFn createFn)
{
...
void LEDPixelsSimple::getPatternNames(std::vector<String>& patternNames) const
{
...
int32_t LEDPixelsSimple::getSegmentIdx(const String& segmentName) const
{
...
void LEDPixelsSimple::clear(bool showAfterClear)
{
    // Clear all segments
    for (auto& segment : _segments)
    {
...
void LEDPixelsSimple::waitUntilShowComplete()
{
    for (auto& ledStrip : _ledStripDrivers)
    {

...
R"(
    isDefined1 = defined($globalVar1)
    isDefined2 = defined($globalVar2)
    if: isDefined1
...
TEST_CASE("Expression test 4", "[expressions]")
{
    ...
    // Eval a second time
    LOG_I(MODULE_PREFIX, "Expressions test 4 - eval2");
    evaluator.evalStatements("");

    // Check values
    TestElem varAssertions2 [] = {
        { "isDefined1", 1 },
        { "isDefined2", 1 },
        { "$globalVar1", 1075 },
        { "$globalVar2", 13 }
    };
    for (int testIdx = 0; testIdx < sizeof(varAssertions2)/sizeof(varAssertions2[0]); testIdx++)
    {
        String tokStartStr = "testExpressions4-2 finalTestIdx=" + String(testIdx);
        TEST_ASSERT_MESSAGE(checkVal(evaluator, varAssertions2[testIdx].varName, 
                        varAssertions2[testIdx].expVal), tokStartStr.c_str());
    }
